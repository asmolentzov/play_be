...
const update = (request, response) => {
  ...
  playlist.update(request.params.id, newPlaylist)
    .then(newPlaylist => {
      if(newPlaylist.length) {
        response.status(200).json(newPlaylist);
      } else {
        response.status(400).json({ error: `Could not find Playlist with ID: ${request.params.id}.`});
      }
    })
    .catch(error => {
      response.status(500).json({ error });
